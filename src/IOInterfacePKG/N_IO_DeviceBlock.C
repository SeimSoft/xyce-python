...
bool DeviceBlock::extractYDeviceData( const TokenVector & parsedInputLine )
{
  ...
  ExtendedString netlistChar(parsedLine_[0].string_.substr(0,1));
  netlistChar.toUpper();
  setName( netlistChar + deviceType + "!" + deviceName );
...
int DeviceBlock::checkIfModelValid(const std::string & modelType, int modelLevel, int fieldNo)
{
  ...
  std::cout << "DEBUG checkIfModelValid: device_type=" << device_type 
            << ", modelType=" << modelType 
            << ", modelLevel=" << modelLevel 
            << ", isModelLevelValid=" << model_metadata.isModelLevelValid() 
            << ", isModelTypeValid=" << model_metadata.isModelTypeValid(modelType) 
            << ", numNodes=" << model_metadata.numNodes
            << std::endl;
  ...
  if ( device_type != "K")
  {
    ...
    {          
      ...
      if (fieldNo >= model_metadata.numNodes+1) { returnValue=1; }
